...
for(R1 in 1:dim(fullmatrix3)[2]){
  ...
  shared_sum = sum(shared_enriched) #Number of shared peptides to remove from N for null denominator calculation
  
  
  
  uniques = fullmatrix3[shared_enriched!=1,R1]  #Vector of all Vi assocated peptides not assocated with virus j
...
full_prob = as.data.frame(spread(temp,unique_table2,unique_table))
fwrite((as.data.frame(full_prob)),"/Users/danielmonaco1/Desktop/PythonAvarda/RVARDA_master/RVARDA_git/bin/unique_probabilities2.csv")
```
