...
AVARDA = function(case_path,thresh,dict_path,total_path,pairwise_path,blast_path,out_path){
  ...
  case = data.frame(fread(case_path, data.table = FALSE)) # this is the case of interest to be analyzed - changeable
  
  filter  = function(edge,vertex){ #independence filter that takes a dictionary (defined above) and a set of nodes and tells the minimal number of unique epitopes
    nodes = unlist(vertex)
  ...
  total_calc  = function(case,column,thresh,total,blast){ # subsetting function that also ranks peptides
    ...
    blast_subset = subset(blast,row.names(blast) %in% row.names(enriched)) # subset fullmatrix to only include peptides above threshold
    blast_subset=blast_subset[, colSums(ifelse(blast_subset>0, 1, 0)) > 7] # subset full matrix to only include viruses with at least three enriched peptide
    ###
    ...
    results = fullmatrix_sorted[,order(unlist(order))] #reorder the subset virus-peptide blast matirx by likelihood of infection
    return(results)
  }
